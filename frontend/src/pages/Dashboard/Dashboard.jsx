...
const Dashboard = () => {
  ...
  const allUsers = (stats?.recentUsers?.length ? stats.recentUsers : fallbackUsers)
    .slice(0, 8)
    .map((user, index) => ({
      ...user,
      id: user.id || `user-${index + 1}`,
      role: user.role || (index % 2 ? "Student" : "Admin"),
      status: user.status || "Active",
      joined: user.joined || user.date || "Recently",
            ...
            <section className="space-y-6">
              <div className="rounded-3xl border border-gray-100 bg-white p-6 shadow-sm">
                <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
                  <div>
                    <p className="text-sm uppercase tracking-[0.3em] text-indigo-500">User Management</p>
                    <h2 className="mt-2 text-2xl font-bold text-gray-900">All Users</h2>
                    <p className="mt-2 text-sm text-gray-500">Track registered users, roles, activity, and account status from a single place.</p>
                  </div>
                  <div className="flex flex-wrap items-center gap-3">
                    <button className="inline-flex items-center gap-2 rounded-xl border border-gray-200 bg-gray-50 px-4 py-2 text-sm font-semibold text-gray-700 hover:bg-gray-100"> <FaFilter className="text-xs" /> Filter </button>
                    <button className="inline-flex items-center gap-2 rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white shadow-sm hover:bg-indigo-700"> <FaPlus className="text-xs" /> Add User </button>
                    <button className="inline-flex items-center gap-2 rounded-xl border border-indigo-200 bg-indigo-50 px-4 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100"> <FaDownload className="text-xs" /> Export </button>
                  </div>
                </div>

                <div className="mt-5 flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
                  <label className="flex items-center gap-3 rounded-2xl border border-gray-200 bg-gray-50 px-4 py-3 text-sm text-gray-500 shadow-sm xl:min-w-[320px]">
                  ...
                  <div className="flex flex-wrap gap-2 text-xs font-semibold text-gray-500">
                    {['All Users', 'Active', 'Pending', 'Admins'].map((chip) => (
                      <button key={chip} className={`rounded-full border px-3 py-1.5 transition ${chip === 'All Users' ? 'border-indigo-200 bg-indigo-50 text-indigo-700' : 'border-gray-200 bg-white hover:border-indigo-200 hover:text-indigo-700'}`}>
                        {chip}
                      </button>
                    ))}
                  </div>
              ...
              <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                {[
                  { label: 'Total Users', value: '1,248', icon: <FaUsers className="text-indigo-600" />, tone: 'bg-indigo-50 text-indigo-700' },
                  { label: 'Active Accounts', value: '1,107', icon: <FaCheckCircle className="text-emerald-600" />, tone: 'bg-emerald-50 text-emerald-700' },
                  { label: 'Pending Approval', value: '42', icon: <FaUserClock className="text-amber-600" />, tone: 'bg-amber-50 text-amber-700' },
                  { label: 'Admin Users', value: '18', icon: <FaUserShield className="text-rose-600" />, tone: 'bg-rose-50 text-rose-700' },
                ].map((item) => (
                  <article key={item.label} className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
                    <div className="flex items-start justify-between">
                      ...
                      <span className={`rounded-2xl p-3 ${item.tone}`}>{item.icon}</span>
                    </div>
                  </article>
                ))}
              </div>
              ...
              <div className="rounded-3xl border border-gray-100 bg-white p-5 shadow-sm">
                <div className="mb-4 flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-sm text-gray-500">A quick overview of the latest student and admin accounts.</p>
                  </div>
                  <button className="rounded-xl border border-indigo-200 bg-indigo-50 px-3 py-2 text-sm font-semibold text-indigo-700 hover:bg-indigo-100">View all</button>
                </div>
                    ...
                    <tbody className="divide-y divide-gray-100">
                      {allUsers.map((user) => (
                        <tr key={user.id} className="hover:bg-gray-50/70">
                          ...
                          <td className="py-4 pr-4 text-gray-600">{user.role}</td>
                          <td className="py-4 pr-4">
                            <span className={`inline-flex rounded-full px-3 py-1 text-xs font-semibold ${user.statusColor}`}>{user.status}</span>
                          </td>
                          <td className="py-4 pr-4 text-gray-600">{user.joined}</td>
                          <td className="py-4 pr-4 text-gray-600">{user.lastLogin}</td>
                          <td className="py-4 text-right">
                            <button className="rounded-lg border border-gray-200 bg-white px-3 py-1.5 text-xs font-semibold text-gray-700 hover:bg-gray-100">Manage</button>
                          </td>
                        </tr>
                      ))}
                    </tbody>
